'use strict';
/**
 * CDP-based two-stage official URL discovery for RankingSelect recovery.
 *
 * Stage 1: Visit homepage → find master's/graduate catalog links
 * Stage 2: Visit catalog → find engineering/CS program links + requirements/deadline/docs/language
 *
 * Saves raw HTML + manifest for every visit. No fake success for blocked.
 */
...
const RAW_DIR = path.join(ROOT, 'scraper/raw/official-discovery/recovery-v5');
const OUTPUT_DIR = path.join(ROOT, 'scraper/playwright/recovery_v5');
const QUEUE_FILE = path.join(ROOT, 'scraper/playwright/top500_recovery_queue_v5.json');
const sleep = ms => new Promise(r => setTimeout(r, ms));

// Aggregator/denied hosts - never collect these
const DENIED_HOSTS = [
...
// Link keywords for master's/graduate catalog (stage 1)
const CATALOG_KEYWORDS = [
  'master','graduate','postgraduate','msc','meng','m.phil','mphil',
  'program','programme','course','degree','study','admission','apply',
  'tuition','curriculum','catalog','prospectus',
];
// Link keywords for engineering/CS (stage 2)
...
// Link keywords for requirements/deadline/docs/language
...
function isSameDomain(url, baseDomain) {
  try { return new URL(url).hostname.toLowerCase().endsWith(baseDomain); }
  catch { return false; }
}
...
function saveRaw(cid, kind, html, url, finalUrl, status, title, extra = {}) {
  ...
  const manifest = {
    schemaVersion: 1,
    kind,
    captureType: 'cdp-browser-rendered',
    requestedUrl: url,
    finalUrl,
    title: title || null,
...
async function navigateAndWait(c, url, waitMs = 3000) {
  try {
    const navResult = await c.send('Page.navigate', { url });
    if (navResult.errorText) {
      return { status: 'error', error: navResult.errorText, finalUrl: url };
    }
  } catch (e) {
    return { status: 'error', error: e.message, finalUrl: url };
  }
  await sleep(waitMs);
  // Get page info
...
async function processSchool(c, item) {
  ...
  // Stage 0: Get the homepage URL
  let homeUrl = item.homepageUrl;
  if (!homeUrl && item.domainGuesses && item.domainGuesses.length > 0) {
    homeUrl = item.domainGuesses[0];
  }
  if (!homeUrl) {
    result.errors.push('no homepage URL and no domain guess');
    return result;
  }
  ...
  if (!homeResult.html || homeResult.ready === 'loading') {
    await sleep(2000);
  }

  const homeManifest = saveRaw(cid, 'homepage', homeResult.html, homeUrl, homeResult.finalUrl, homeResult.ready === 'complete' ? 200 : 'partial', homeResult.title);
  ...
  const uniqueCatalogLinks = catalogLinks.filter(l => {
    ...
    seen.add(l.href);
    return true;
  }).slice(0, 5); // Visit up to 5 catalog links

  result.discoveredUrls.push({
    url: cleanUrl(homeResult.finalUrl),
    type: 'homepage',
  ...
  for (const cl of uniqueCatalogLinks) {
    ...
    const catManifest = saveRaw(cid, urlType, catResult.html, cl.href, catResult.finalUrl, 200, catResult.title, { linkText: cl.text });
    result.visited.push({ url: cl.href, finalUrl: catResult.finalUrl, status: 200, kind: urlType });
    result.discoveredUrls.push({
      url: cleanUrl(catResult.finalUrl),
      type: urlType,
      title: catResult.title,
      linkText: cl.text,
    });

    // Extract engineering/CS links from catalog page
      ...
      for (const el of engLinks) {
        ...
        if (isUsefulUrl(cleanProgHref) && !result.discoveredUrls.some(d => d.url === cleanProgHref)) {
          result.discoveredUrls.push({
            url: cleanProgHref,
            type: 'program-page',
            title: el.text.slice(0, 100),
            linkText: el.text,
          });
        }
      }
      // Also look for requirements/deadline/docs/language links
...
async function run() {
  ...
  console.log(`Recovery v5: processing ${batch.length} schools (offset=${offset}, limit=${limit})`);

  // Launch Chrome
  const profileDir = os.tmpdir() + '/cdp-recovery-v5';
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    await sleep(500); // Small delay between schools
  }
