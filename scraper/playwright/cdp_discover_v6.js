/**
 * CDP-based two-stage official URL discovery for RankingSelect recovery v6.
...
const RAW_DIR = path.join(ROOT, 'scraper/raw/official-discovery/recovery-v6');
const OUTPUT_DIR = path.join(ROOT, 'scraper/playwright/recovery_v6');
const QUEUE_FILE = path.join(ROOT, 'scraper/playwright/top500_recovery_queue_v6.json');
...
async function run() {
  ...
  console.log(`Recovery v6: processing ${batch.length} schools (offset=${offset}, limit=${limit})`);

  const profileDir = os.tmpdir() + '/cdp-recovery-v6';
