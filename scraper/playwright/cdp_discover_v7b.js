/**
 * CDP-based two-stage official URL discovery for RankingSelect recovery v7b.
...
const RAW_DIR = path.join(ROOT, 'scraper/raw/official-discovery/recovery-v7b');
const OUTPUT_DIR = path.join(ROOT, 'scraper/playwright/recovery_v7b');
const QUEUE_FILE = path.join(ROOT, 'scraper/playwright/top500_recovery_queue_v7b.json');
...
// Parse links from HTML directly (more reliable than CDP evaluation)
function extractLinksFromHtml(html, baseUrl) {
  ...
  while ((m = re.exec(html))) {
    let href = m[1];
    let text = m[2].replace(/<[^>]*>/g, '').trim().slice(0, 200);
    // Resolve relative URLs
    if (href.startsWith('/')) {
      if (base) href = base.origin + href;
    } else if (!href.startsWith('http') && !href.startsWith('mailto:') && !href.startsWith('tel:') && !href.startsWith('javascript:') && !href.startsWith('#')) {
      if (base) {
        try { href = new URL(href, base.href).href; } catch { continue; }
      } else continue;
    }
    if (!href.startsWith('http')) continue;
    if (href.includes('#')) continue;
    if (/\.(pdf|jpg|jpeg|png|gif|svg|css|js|ico|woff|woff2|ttf|mp4|mp3|zip|rar)(\?|$)/i.test(href)) continue;
...
async function extractLinks(c, baseUrl) {
  // Fallback: use CDP evaluation if needed
  try {
    ...
    const cdpLinks = JSON.parse(result.result.value);
    if (cdpLinks && cdpLinks.length > 0) return cdpLinks;
  } catch {}
  return [];
}
...
async function processSchool(c, item) {
  ...
  // Use finalUrl (after redirect) for domain matching
  let baseDomain = (() => { try { return new URL(homeResult.finalUrl || homeUrl).hostname; } catch { try { return new URL(homeUrl).hostname; } catch { return ''; } } })();
  ...
  // Retry for SPA pages: if HTML is very small, wait longer and re-evaluate
  if (homeResult.html && homeResult.html.length < 5000 && homeResult.ready !== 'complete') {
    await sleep(5000);
    try {
      const retryResult = await c.send('Runtime.evaluate', {
        expression: 'JSON.stringify({url: location.href, title: document.title, html: document.documentElement.outerHTML, ready: document.readyState})',
        returnByValue: true,
      });
      const retryInfo = JSON.parse(retryResult.result.value);
      if (retryInfo.html && retryInfo.html.length > homeResult.html.length) {
        homeResult.html = retryInfo.html;
        homeResult.title = retryInfo.title;
        homeResult.finalUrl = retryInfo.url;
        homeResult.ready = retryInfo.ready;
        // Update baseDomain with final URL
        try { baseDomain = new URL(retryInfo.url).hostname; } catch {}
        // Note: baseRegDomain not updated, but isSameDomain uses getRegistrableDomain(baseDomain) which is called at filter time
      }
    } catch {}
  }
  ...
  const links = extractLinksFromHtml(homeResult.html, homeResult.finalUrl).concat(await extractLinks(c, homeResult.finalUrl));
  // Deduplicate
  const linkSeen = new Set(); const dedupLinks = [];
  for (const l of links) { if (!linkSeen.has(l.href)) { linkSeen.add(l.href); dedupLinks.push(l); } }
  const allLinks = dedupLinks;
  const sameDomainLinks = allLinks.filter(l => !isDenied(l.href) && isSameDomain(l.href, baseDomain));
  ...
  for (const cl of uniqueCatalogLinks) {
    ...
    if (urlType === 'master-catalog' || urlType === 'engineering-cs-catalog') {
      const catLinks = extractLinksFromHtml(catResult.html, catResult.finalUrl).concat(await extractLinks(c, catResult.finalUrl));
      const catSeen = new Set(); const catDedup = [];
      for (const l of catLinks) { if (!catSeen.has(l.href)) { catSeen.add(l.href); catDedup.push(l); } }
      const catAllLinks = catDedup;
      const engLinks = catAllLinks.filter(l => !isDenied(l.href) && isSameDomain(l.href, baseDomain) && matchKeywords(l.text + ' ' + l.href, ENG_CS_KEYWORDS));
...
async function run() {
  ...
  console.log(`Recovery v7b: processing ${batch.length} schools (offset=${offset}, limit=${limit})`);

  const profileDir = os.tmpdir() + '/cdp-recovery-v7bb';
